...
def _process_summary(segments, top_k, threshold, output_file):
    """Process segments into summary using clustering."""
    import re
...
def _summarize_file_impl(input_data: dict) -> dict:
    """Common implementation for summarize_file_content across all platforms."""
    import os
    import re
    import sys
    import importlib
    ...
    try:
        ...
        # Ensure dependencies
        for pkg in ['scikit-learn', 'sentence-transformers']:
            try:
        ...
        segments = _chunk_text(content, chunk_size=300, overlap=50)
        ...
        if not segments:
            ...
            return {
                'status': 'success',
                'message': 'File was empty, created empty summary',
                'summary_file': output_file
            }
        ...
        summary_file = _process_summary(segments, top_k, threshold, output_file)
...
@action(
    ...
    description="Reads a text file and writes a summary to a new text file using semantic clustering.",
    mode="CLI",
    platforms=["linux"],
...
def summarize_file_content_linux(input_data: dict) -> dict:
    return _summarize_file_impl(input_data)
